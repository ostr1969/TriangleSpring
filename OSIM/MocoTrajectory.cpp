...
void MocoTrajectory::removeState(const std::string& name){
    ensureUnsealed();
    auto it = std::find(
            m_state_names.cbegin(), m_state_names.cend(), name);
    OPENSIM_THROW_IF(it == m_state_names.cend(), Exception,
            "Cannot find state named {}.", name);
    int index = (int)std::distance(m_state_names.cbegin(), it);
    const int numTimes = m_time.size();
    int numStates = (int)m_state_names.size();
    SimTK::Matrix m_states_copy;
    std::vector<std::string> m_state_names_copy;
    int i=0,j=0;
    for (const auto& trajectory_state : m_state_names) {
	    if (j!=index){
	m_state_names_copy.push_back(trajectory_state);	    
        m_states_copy.resizeKeep(numTimes, i+1);
	m_states_copy.updCol(i)=m_states.updCol(j);
	    i++;}
	    j++;}
    m_states.resize(numTimes,numStates-1);
    m_states=m_states_copy;
    m_state_names=m_state_names_copy;
}
void MocoTrajectory::removeStates(std::vector<std::string> names){
	 for (const auto& cn : names) {
	removeState(cn);}
}
void MocoTrajectory::removeControl(const std::string& name){
    ensureUnsealed();
    auto it = std::find(
            m_control_names.cbegin(), m_control_names.cend(), name);
    OPENSIM_THROW_IF(it == m_control_names.cend(), Exception,
            "Cannot find control named {}.", name);
    int index = (int)std::distance(m_control_names.cbegin(), it);
    const int numTimes = m_time.size();
    int numControls = (int)m_control_names.size();
    SimTK::Matrix m_controls_copy;
    std::vector<std::string> m_control_names_copy;
    int i=0,j=0;
    for (const auto& trajectory_control : m_control_names) {
	    if (j!=index){
	m_control_names_copy.push_back(trajectory_control);	    
        m_controls_copy.resizeKeep(numTimes, i+1);
	m_controls_copy.updCol(i)=m_controls.updCol(j);
	    i++;}
	    j++;}
    m_controls.resize(numTimes,numControls-1);
    m_controls=m_controls_copy;
    m_control_names=m_control_names_copy;
}
void MocoTrajectory::removeControls(std::vector<std::string> names){
	 for (const auto& cn : names) {
	removeControl(cn);}
}

void MocoTrajectory::appendParameter(
        const std::string& name, const double value) {
    ensureUnsealed();

    OPENSIM_THROW_IF(m_time.nrow() == 0, Exception,
            "The time vector must be set before adding parameter variables.");

  //  SimTK::Vector trajectory(m_time.nrow(),value);
  //  OPENSIM_THROW_IF(trajectory.size() != m_time.nrow(), Exception,
  //          "Attempted to add parameter {} of length {}, but it is incompatible "
  //          "with the time vector, which has length {}.",
  //          name, trajectory.size(), m_time.nrow());

    m_parameter_names.push_back(name);
    m_parameters.resizeKeep(m_parameters.size() + 1);
    setParameter(name,value);
}
void MocoTrajectory::appendControls(std::vector<std::string> control_names,double value){
    const int numTimes = m_time.size();
    for (const auto& c : control_names) {
        int numControls= (int)m_control_names.size();
	m_controls.resizeKeep(numTimes,numControls+1);
	m_control_names.push_back(c);
        SimTK::Vector cval(numTimes,value);
        setControl(c,cval);}
...
void MocoTrajectory::resample(SimTK::Vector time) {
    ...
    //std::cout<<(time[time.size() - 1] - m_time[m_time.size() - 1])<<std::endl;
    if (abs(time[time.size() - 1] - m_time[m_time.size() - 1])<1e-10)
