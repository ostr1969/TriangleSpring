...
void DeGrooteFregly2016Muscle::constructProperties() {
    ...
    constructProperty_coordRef1 ("0");
    constructProperty_coordRef2 ("0");
}
...
void DeGrooteFregly2016Muscle::computeStateVariableDerivatives(
	...
	if (actTimeConst==0.111)
	cout<<getName()+":"+std::to_string(actTimeConst)+","+
		std::to_string(deactTimeConst)+","+
		std::to_string(timeConstFactor)+","+
		std::to_string(tempAct)+","+
		std::to_string(tempDeact)+","+
		std::to_string(f) +","+
		std::to_string(timeConst)+","+
...
double DeGrooteFregly2016Muscle::showControl ( const SimTK::State& s ) const
{
		    return getControl(s);
}
double DeGrooteFregly2016Muscle::computeArm1( const SimTK::State& s ) const
{
	string coordRef=get_coordRef1();
	if (coordRef=="0")
			return 0;
	else {
...
double 	DeGrooteFregly2016Muscle::computeTorque1 ( const SimTK::State& s ) const
{
		    return computeActuation(s)*computeArm1(s);
}
double DeGrooteFregly2016Muscle::computeArm2( const SimTK::State& s ) const
{
	string coordRef=get_coordRef2();
	if (coordRef=="0")
			return 0;
	else {
	const Coordinate& aCoord = getModel().getCoordinateSet().get(coordRef);
...
double 	DeGrooteFregly2016Muscle::computeTorque2 ( const SimTK::State& s ) const
{
		    return computeActuation(s)*computeArm2(s);
}
...
void DeGrooteFregly2016Muscle::replaceMuscles(
    ...
    for (int im = 0; im < muscleSet.getSize(); ++im) {
	 ...
	 //   if (musc->get_activation_time_constant()<0.002)
	//	    cout<<std::to_string(musc->get_activation_time_constant())+"\n";
