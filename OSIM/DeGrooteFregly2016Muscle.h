...
public:
    ...
    OpenSim_DECLARE_PROPERTY(coordRef1 , std::string,"The joint where the arm and torque"
		    " Will be calculated when asked as output")
    OpenSim_DECLARE_OUTPUT(arm1, double, computeArm1,
                           SimTK::Stage::Acceleration);
    OpenSim_DECLARE_OUTPUT(torque1, double, computeTorque1,
                           SimTK::Stage::Acceleration);
    OpenSim_DECLARE_OUTPUT(control, double, showControl,
                           SimTK::Stage::Acceleration);
    OpenSim_DECLARE_PROPERTY(coordRef2 , std::string,"The joint where the arm and torque"
		    " Will be calculated when asked as output")
    OpenSim_DECLARE_OUTPUT(arm2, double, computeArm2,
                           SimTK::Stage::Acceleration);
    OpenSim_DECLARE_OUTPUT(torque2, double, computeTorque2,
                           SimTK::Stage::Acceleration);
...
protected:
    ...
    double computeArm1      ( const SimTK::State& s) const ;
    double computeTorque1   ( const SimTK::State& s) const ;
    double computeArm2      ( const SimTK::State& s) const ;
    double computeTorque2   ( const SimTK::State& s) const ;
