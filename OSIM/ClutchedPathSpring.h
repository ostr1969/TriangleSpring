...
//=============================================================================
    ...
    OpenSim_DECLARE_PROPERTY(initial_length , double,
        "The initial length  (m) of the spring element. Note if the length is "
        "not specified, the actuator will not restore to original force if reengaged, "
        "if initial length is not 0 the spring will return to its original force after "
	"reengaged");
    OpenSim_DECLARE_PROPERTY(coordRef , std::string,"The joint where the arm and torque"
		    " Will be calculated when asked as output")
    OpenSim_DECLARE_OUTPUT(length, double, computeLength,
                           SimTK::Stage::Acceleration);
    OpenSim_DECLARE_OUTPUT(arm, double, computeArm,
                           SimTK::Stage::Acceleration);
    OpenSim_DECLARE_OUTPUT(stretch, double, computeStretch,
                           SimTK::Stage::Acceleration);
    OpenSim_DECLARE_OUTPUT(torque, double, computeTorque,
                           SimTK::Stage::Acceleration);
    OpenSim_DECLARE_OUTPUT(control, double, showControl,
                           SimTK::Stage::Acceleration);
    OpenSim_DECLARE_OUTPUT(ncontrol, double, showNControl,
                           SimTK::Stage::Acceleration);
...
//=============================================================================
     ...
     * @param length0      the initial length  of the spring in m */
    ...
    double getInitialLength() {return get_initial_length(); }
    void setInitialStretch(double stretch0);
    void setInitialLength  (double length0);
...
protected:
    ...
    double computeLength   ( const SimTK::State& s) const ;
    double computeArm      ( const SimTK::State& s) const ;
    double computeStretch  ( const SimTK::State& s) const ;
    double computeTorque   ( const SimTK::State& s) const ;
    double showControl     ( const SimTK::State& s) const ;
    double showNControl    ( const SimTK::State& s) const ;
